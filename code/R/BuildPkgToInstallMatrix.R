...
PkgsToInstall = matrix(
  ...
  byrow = TRUE,
  dimnames = list(
    indices = seq(length(pkgDatMatrix) / pkgDatCols),
    params = c("pkNames", "force" ,"dependencies" ,"build_vignettes" , "build_manuals")
  )
)

PkgsToInstall = matrix(
  data = as.logical(PkgsToInstall[, -1]),
  ncol = pkgDatCols - 1,
  byrow = TRUE,
  dimnames = list(
    repos = as.character(PkgsToInstall[, 1]),
    params = c("force" ,"dependencies" ,"build_vignettes" , "build_manuals")
  )
)
